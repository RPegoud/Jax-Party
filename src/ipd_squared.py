...
def register_IPDSquared():
    ...
    register("IPDSquared-v0", "src.ipd_squared:IPDSquared")
...
class IPDSquared(Environment[State, specs.DiscreteArray, Observation]):
    ...
    def __init__(
        ...
        epsilon_max: float,
        scaling_factor: int,
        outer_action_selection: str,  # "max" or "sampling"
        time_limit: int = 1000,
        cc: float = 1,
        cd: float = 4,
        dd: float = -1,
        ...
        self.outer_action_selection = outer_action_selection
    ...
    def _select_outer_actions(
        self, key: chex.PRNGKey, inner_actions: chex.Array, power: chex.Array
    ) -> chex.Array:
        if self.outer_action_selection == "max":
    ...
    def step(
        ...
        epsilons = jnp.zeros((2, 1))

        power = jax.nn.softmax(state.power + epsilons, axis=-1)
        outer_actions = self._select_outer_actions(
            action_key, inner_actions, power
        ).squeeze()
